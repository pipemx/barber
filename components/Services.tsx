    ...
    <Reveal delay={index * 0.08}>
      <motion.button
        ...
        className={`group relative w-full h-full text-left rounded-2xl p-7 md:p-8 cursor-pointer transition-shadow duration-300 flex flex-col gap-4 ${
          featured
            ? "bg-surface-2 border-2 border-gold/60 shadow-glow-gold"
            : "bg-surface border border-line hover:border-gold/40"
        }`}
      ...
      >
        {service.badge && (
          <span
            className={`absolute -top-3 left-6 rounded-full px-4 py-1 text-[11px] font-semibold uppercase tracking-widest ${
              featured ? "btn-shimmer text-ink" : "bg-gold-deep text-cream"
            }`}
          >
            {service.badge}
          </span>
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="text-right shrink-0">
            <span className="font-display text-2xl md:text-3xl text-gold-bright tabular-nums">
              ${service.price}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-line/60">
          ...
          <span className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-gold group-hover:text-gold-bright transition-colors">
            Reservar
    ...
    <section id="servicios" className="relative py-24 md:py-32 px-5 md:px-8 section-glow">
      <div className="mx-auto max-w-6xl">
        <SectionHeading
          ...
          title={
            <>
              Precisión que se <span className="text-gold-gradient italic">nota</span>
            </>
          }
          subtitle="Cada servicio incluye consulta personalizada, productos premium y el tiempo que mereces. Sin prisas, sin filas."
        />
