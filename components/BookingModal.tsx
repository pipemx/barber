      ...
      {open && (
        ...
        >
          <motion.div
            ...
            className="w-full sm:max-w-lg max-h-[92dvh] overflow-y-auto bg-surface border border-gold/25 rounded-t-3xl sm:rounded-3xl shadow-card"
          >
            {/* Header */}
              ...
              <div className="flex items-center gap-3">
                {step > 0 && (
                  <button
                    ...
                    className="w-9 h-9 rounded-full border border-line flex items-center justify-center text-stone-muted hover:text-gold hover:border-gold/50 transition-colors cursor-pointer"
                ...
                <div>
                  <p className="text-[10px] text-gold uppercase tracking-[0.25em]">
                    Paso {step + 1} de 4
                  </p>
                ...
                >
                        ...
                        >
                          <div>
                            ...
                            <p className="flex items-center gap-1.5 text-xs text-stone-muted mt-1">
                              <Clock className="w-3 h-3 text-gold" /> {s.duration} min
                            </p>
                          </div>
                          <span className="font-display text-xl text-gold-bright tabular-nums">
                            ${s.price}
                          </span>
                        </button>
                      ))}
                  ...
                  {step === 1 && (
                    <div className="flex flex-col gap-3">
                      {barbers.map((b) => (
                        ...
                        >
                          <span className="w-11 h-11 rounded-full bg-gold/10 border border-gold/30 flex items-center justify-center font-display text-gold text-sm shrink-0">
                            {b.initials}
                          </span>
                          <div>
                        ...
                        <div className="flex gap-2 overflow-x-auto pb-2 -mx-1 px-1">
                          {days.map((d, i) => (
                            ...
                            >
                              ...
                              <span className="text-[10px] uppercase text-gold">{d.month}</span>
                            </button>
                          ))}
                        </div>
                      </div>
                            ...
                            <button
                              ...
                              className={`rounded-lg border py-2.5 text-sm tabular-nums cursor-pointer transition-colors duration-200 ${
                                time === t
                                  ...
                                  : "border-line bg-ink-soft text-cream hover:border-gold/40"
                              }`}
                            >
                              {t}
                            </button>
                  ...
                  {step === 3 && service && barber && date && time && (
                    <div className="flex flex-col gap-5">
                      <div className="bg-ink-soft border border-gold/25 rounded-2xl p-5 flex flex-col gap-3">
                        {[
                      ...
                      <div>
                        ...
                        <input
                          ...
                          autoComplete="name"
                          className="w-full rounded-xl bg-ink-soft border border-line focus:border-gold outline-none px-4 py-3.5 text-cream placeholder:text-stone-muted/50 transition-colors"
                        />
